...
\setlength{\tabcolsep}{6pt}
    ...
    \toprule 
    Task & \makecell{BERT\\{\small BASE}} & Spars. & \makecell{Soft\\MvP} & \makecell{oBERT\\(ours)} & \makecell{LT-\\BERT} & \makecell{oBERT\\(ours)}\\
    \midrule
...
% We presented the first approximate second-order pruning method applied at the BERT scale, and investigated its practicality in the context of compounding compression techniques. 
...
\comment{ % commented for arxiv release
\subsection{Responsible NLP Research}

\subsubsection{Limitations}
While our methods consistently work across the datasets which we discussed in the paper, there are known and unknown limitations to our work. 
One limitation, which we aim to address in future work, concerns the impact of compression techniques on secondary metrics besides accuracy of the resulting models, in particular metrics  connected to bias. Specifically, in future work we aim to investigate the proclivity towards bias specifically for compressed models, along with potential countermeasures. Another limitation of our submission is the impact of multi-linguality on compression. As our experiments focus on English datasets, we do not have empirical evaluation on the impact compression has on multi-lingual language models. While we believe our methods likely work well, it is possible that compression causes a non uniform degradation across languages and language groups.

\subsubsection{Risks}
As discussed in our limitation it is possible that our compression methods lead to models with bias or degradation of quality across languages. Since compressed models effectively try to remove unnecessary computation in a network, this may make networks more brittle and more susceptible to adversarial attacks. We believe these risks are real and as a result provide a compelling and important thread of research to explore in future work. 
